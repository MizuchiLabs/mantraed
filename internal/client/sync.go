...
import (
	...
	"github.com/mizuchilabs/mantraed/internal/collector"
	mantraev1 "github.com/mizuchilabs/mantraed/internal/gen/mantrae/v1"
	"github.com/mizuchilabs/mantraed/internal/gen/mantrae/v1/mantraev1connect"
...
func (s *SyncJob) upsertRouter(
	...
	res, err := s.routerClient.ListRouters(ctx, &mantraev1.ListRoutersRequest{
	...
	slog.Debug("Created router", "name", createRes.Router.Name, "id", createRes.Router.Id)
	return nil
}
...
func (s *SyncJob) upsertService(
	...
	res, err := s.serviceClient.ListServices(ctx, &mantraev1.ListServicesRequest{
		ProfileId: s.config.ProfileID,
		AgentId:   &s.config.AgentID,
		Type:      &protocol,
	})
	...
	for _, svc := range res.Services {
		if svc.Name == name {
			...
			if _, err = s.serviceClient.UpdateService(
				ctx,
				&mantraev1.UpdateServiceRequest{
					Id:      svc.Id,
					Name:    name,
					Config:  config,
					Type:    protocol,
	...
	createRes, err := s.serviceClient.CreateService(
		ctx,
		&mantraev1.CreateServiceRequest{
			ProfileId: s.config.ProfileID,
			AgentId:   &s.config.AgentID,
			Name:      name,
			Config:    config,
	...
	slog.Debug(
		"Created service",
		"name",
		createRes.Service.Name,
		"id",
		createRes.Service.Id,
	)
	return nil
}
...
func (s *SyncJob) upsertMiddleware(
	...
	res, err := s.middlewareClient.ListMiddlewares(
		ctx,
		&mantraev1.ListMiddlewaresRequest{
			ProfileId: s.config.ProfileID,
			AgentId:   &s.config.AgentID,
			Type:      &protocol,
		},
	...
	for _, mw := range res.Middlewares {
		if mw.Name == name {
			...
			if _, err = s.middlewareClient.UpdateMiddleware(
				ctx,
				&mantraev1.UpdateMiddlewareRequest{
					Id:      mw.Id,
					Name:    name,
					Config:  config,
					Type:    protocol,
	...
	createRes, err := s.middlewareClient.CreateMiddleware(
		ctx,
		&mantraev1.CreateMiddlewareRequest{
			ProfileId: s.config.ProfileID,
			AgentId:   &s.config.AgentID,
			Name:      name,
			Config:    config,
	...
	slog.Debug(
		"Created middleware",
		"name",
		createRes.Middleware.Name,
		"id",
		createRes.Middleware.Id,
	)
	return nil
}
...
func (s *SyncJob) cleanup(ctx context.Context) error {
	...
	for _, r := range resRouter.Routers {
